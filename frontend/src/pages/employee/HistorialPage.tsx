...
export function HistorialPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white border border-black/5 rounded-2xl shadow-sm p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          <label className="text-xs text-muted block mb-1">Desde</label>
          <input type="date" value={desde} onChange={(e) => setDesde(e.target.value)} className="w-full border border-black/10 rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="text-xs text-muted block mb-1">Hasta</label>
          <input type="date" value={hasta} onChange={(e) => setHasta(e.target.value)} className="w-full border border-black/10 rounded-lg px-3 py-2 text-sm" />
        </div>
        <div>
      ...
      <div className="bg-white border border-black/5 rounded-2xl shadow-sm divide-y divide-black/5">
        {cargando && <p className="text-sm text-muted p-5">Cargando historial...</p>}
        {!cargando && fichajes.map((f) => (
          <div key={f.id} className="flex items-center justify-between px-5 py-3.5 gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted">{new Date(f.fecha_hora_dispositivo).toLocaleString()} · {f.ubicaciones_portales?.nombre ?? 'Edificio'}</p>
            </div>
